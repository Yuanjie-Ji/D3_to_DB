...
var constructSQL = function () {
    ...
    var minH = sliderHumidity.noUiSlider.get()[0];
    var maxH = sliderHumidity.noUiSlider.get()[1];
    var minW = sliderWind.noUiSlider.get()[0];
    var maxW = sliderWind.noUiSlider.get()[1];
    var minT = sliderTemp.noUiSlider.get()[0];
    var maxT = sliderTemp.noUiSlider.get()[1];

    var sqlString = `select * from forestfire where humidity>=${minH} and humidity<=${maxH} and wind>=${minW} and wind<=${maxW} and temp>=${minT} and temp<=${maxT} and day in (${days.join()}) and month in (${months.join()})`;
...
var queryTemplate = "SELECT MIN(X), MAX(X), MIN(Y), MAX(Y), MIN(temp), MAX(temp), MIN(humidity), MAX(humidity), MIN(wind), MAX(wind) FROM forestfire"
